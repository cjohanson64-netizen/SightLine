...
interface PhrasePlannerInput {
  ...
  difficulty: ExerciseSpec['difficulty'] | number;
  cadence: ExerciseSpec['cadence'];
  seed: number;
}
...
function cadenceOptions(cadence: ExerciseSpec['cadence']): number[][] {
  if (cadence === 'half') {
    return [
      [2, 5],
      [4, 5]
...
export function generatePhrasePlan(input: PhrasePlannerInput): PhrasePlan {
  ...
  const startOptions = [1];
  if (difficulty >= 2) {
    startOptions.push(3, 5);
  }
  const startDegree = clampDegree(pick(rng, startOptions), input.range);
  ...
  const cadenceTemplate = pick(rng, cadenceOptions(input.cadence));
