...
import {
  chooseGrid,
  chooseMeasureTemplate,
  classifyMeasureRhythmMode,
  classifyMeasuresForRhythm,
  templateById,
} from './melody/rhythm';
import type {
  CadenceSlotContext,
...
export function normalizeAndValidatePass0(spec: ExerciseSpec): ExerciseSpec {
...
export function parseRangeByScaleDegree(spec: ExerciseSpec): [number, number] {
...
export function generateStructuralSkeleton(input: {
  ...
  for (let localMeasure = 1; localMeasure <= input.phraseLengthMeasures; localMeasure += 1) {
    ...
    const isCadentialWholeMeasure = plannedMeasure?.wholeNoteRole === 'cadence';
    const plannedAnchors = isCadentialWholeMeasure
      ? [1]
      : plannedMeasure?.anchorOnsets?.length
        ? plannedMeasure.anchorOnsets
        : [1, 3].filter((b) => b <= input.beatsPerMeasure);
    for (const beat of plannedAnchors) {
      const cadenceContext: CadenceSlotContext | undefined =
        isCadentialWholeMeasure
          ? localMeasure === input.phraseLengthMeasures
          ...
          : localMeasure === input.phraseLengthMeasures && beat === finalStructuralBeat
            ? { type: input.phraseSpec.cadence, slotTag: 'final' }
            : undefined;
      const harmonyBeat = isCadentialWholeMeasure ? finalStructuralBeat : beat;
...
export function applyIllegalRulesAdjacencyPass(input: {
  events: MelodyEvent[];
  spec: ExerciseSpec;
  keyScale: number[];
  rangeMin: number;
...
export function applyDominantTendencyVoiceLeadingPass(input: {
  events: MelodyEvent[];
  spec: ExerciseSpec;
  keyScale: number[];
  rangeMin: number;
...
export function enforceLeapBudgetPerPhrasePass(
  events: MelodyEvent[],
  ctx: {
    beatsPerMeasure: number;
...
export function realizePhraseGridPitches(input: {
  spec: ExerciseSpec;
  phraseSpec: PhraseSpec;
  phraseIndex: number;
  phraseLengthMeasures: number;
  ...
  for (let anchorIndex = 0; anchorIndex < anchorSlots.length - 1; anchorIndex += 1) {
    ...
    for (let j = 0; j < intermediates.length; j += 1) {
      ...
      const event: MelodyEvent = {
        ...legalDurationPlaceholder(input.spec.userConstraints?.allowedNoteValues),
...
export function countEePairs(events: MelodyEvent[]): number {
  const attacks = filterRenderableAttackEvents(events);
  const measures = [...new Set(attacks.map((event) => event.measure))];
  let pairs = 0;
  for (const measure of measures) {
...
export function noteValueCounts(events: MelodyEvent[]): { W: number; H: number; Q: number; EE: number } {
...
export function validateAllMustPass10(
  events: MelodyEvent[],
...
function legalDurationPlaceholder(
  allowedNoteValues?: Array<'EE' | 'Q' | 'H' | 'W'>
): { duration: string; durationBeats: number } {
  const allowed = new Set<'EE' | 'Q' | 'H' | 'W'>(allowedNoteValues ?? ['EE', 'Q', 'H']);
  if (!allowed.has('Q') && allowed.has('EE')) {
    return { duration: 'eighth', durationBeats: 0.5 };
  }
  if (allowed.has('Q')) {
    return { duration: 'quarter', durationBeats: 1 };
  }
  if (allowed.has('H')) {
    return { duration: 'half', durationBeats: 2 };
  }
  if (allowed.has('W')) {
    return { duration: 'whole', durationBeats: 4 };
  }
  return { duration: 'quarter', durationBeats: 1 };
}
...
function rewriteFallbackGrid(
  ...
  if (Math.abs(beatsPerMeasure - 4) < 0.001) {
    ...
    if (allowed.has('EE')) {
      if (preferredEeWindow === 1) {
        return [1, 1.5, 2, 3];
      }
      if (preferredEeWindow === 3 || preferredEeWindow === 4) {
        return [1, 3, 3.5, 4];
      }
      return [1, 2, 2.5, 3];
    }
  ...
  if (Math.abs(beatsPerMeasure - 3) < 0.001) {
    ...
    if (allowed.has('EE')) {
      return preferredEeWindow === 1 ? [1, 1.5, 2, 3] : [1, 2, 2.5, 3];
    }
    if (allowed.has('H')) {
      return [1, 2];
    }
  ...
  for (const measure of measures) {
    ...
    while (guard < 16) {
      ...
      for (let i = 0; i < attacks.length; i += 1) {
        ...
        const mergeByDemoting = (attack: MelodyEvent): boolean => {
          if (isHardKeepEvent(attack)) {
            return false;
          }
          attack.isAttack = false;
          attack.tieStop = true;
          attack.durationBeats = 0;
          attack.reason = `${attack.reason}|pass10_allowed_note_values_merge`;
          return true;
        };
        const tryMergeForward = (targetDurations: number[]): boolean => {
          const next = attacks[i + 1];
          if (!next) {
            return false;
          }
          const afterNextOnset =
            i + 2 < attacks.length
              ? (attacks[i + 2].onsetBeat ?? attacks[i + 2].beat)
              : beatsPerMeasure + 1;
          const mergedDur = Number((afterNextOnset - onset).toFixed(3));
          if (
            targetDurations.some((value) => Math.abs(mergedDur - value) < 1e-6) &&
            isDurationAllowedByNoteValues(mergedDur, allowed)
          ) {
            return mergeByDemoting(next);
          }
          return false;
        };
        const tryMergeBackward = (targetDurations: number[]): boolean => {
          const prev = attacks[i - 1];
          if (!prev) {
            return false;
          }
          const prevOnset = prev.onsetBeat ?? prev.beat;
          const mergedDur = Number((nextOnset - prevOnset).toFixed(3));
          if (
            targetDurations.some((value) => Math.abs(mergedDur - value) < 1e-6) &&
            isDurationAllowedByNoteValues(mergedDur, allowed)
          ) {
            return mergeByDemoting(curr);
          }
          return false;
        };
        ...
        if (Math.abs(dur - 4) < 1e-6) {
          ...
          } else if (allowed.has('EE')) {
            changed = insertAt(onset + 0.5) || changed;
            changed = insertAt(onset + 1) || changed;
            changed = insertAt(onset + 1.5) || changed;
            changed = insertAt(onset + 2) || changed;
            changed = insertAt(onset + 2.5) || changed;
            changed = insertAt(onset + 3) || changed;
            changed = insertAt(onset + 3.5) || changed;
          ...
          if (allowed.has('Q')) {
            ...
            changed = insertAt(onset + 1) || changed;
            changed = insertAt(onset + 1.5) || changed;
          } else if (allowed.has('W')) {
            changed = tryMergeForward([4]) || tryMergeBackward([4]) || changed;
          }
        } else if (Math.abs(dur - 1) < 1e-6) {
          if (allowed.has('EE')) {
            changed = insertAt(onset + 0.5) || changed;
            ...
            if (allowed.has('H')) {
              targetDurations.push(2);
            }
            if (allowed.has('W')) {
              targetDurations.push(4);
            }
            changed =
              tryMergeForward(targetDurations) ||
              tryMergeBackward(targetDurations) ||
              changed;
          }
        }
        ...
        if (!isDurationAllowedByNoteValues(dur, allowed)) {
          repairLog.push({
            code: 'pass10_allowed_note_values_unresolved',
            detail: { measure, onset, duration: dur, allowed: [...allowed] }
          });
        }
      }
...
export function enforceEePairMelodicRules(
  events: MelodyEvent[],
  spec: ExerciseSpec,
  tonicPc: number,
  keyScale: number[],
...
export function rewriteAttacksAndQuantizeByMeasure(
  ...
  for (const [measure, inMeasureUnsorted] of byMeasure.entries()) {
    ...
    if (hardKeepIndices.length > grid.length) {
      grid = rewriteFallbackGrid(
        beatsPerMeasure,
        options.allowedNoteValues,
        options.eeWindowBeat
      );
    }
...
export function toRelativePhraseEvents(events: MelodyEvent[], phraseStartMeasure: number): MelodyEvent[] {
...
export function toAbsolutePhraseEvents(
  eventsRelative: MelodyEvent[],
...
export function mergePrimeWithBaseFirstHalf(
