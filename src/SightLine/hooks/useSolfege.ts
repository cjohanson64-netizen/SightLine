...
import { KEY_TO_PC } from "../core/midi";
...
export type SolfegeColorizeMode = "off" | "lyrics" | "full";
...
export function addSolfegeLyricsToMusicXml(
  ...
  try {
    ...
    for (const noteNode of Array.from(doc.querySelectorAll("note"))) {
      ...
      if (options.colorizeLyrics !== false && color) {
...
export function useSolfege() {
  ...
  const [solfegeColorizeMode, setSolfegeColorizeMode] =
    useState<SolfegeColorizeMode>("off");
  const solfegeOverlayMode = solfegeColorizeMode === "full";
