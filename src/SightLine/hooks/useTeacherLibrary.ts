...
import type { AssessmentLogRecord } from "@/SightLine/core/assessmentLogs/types";
...
export type RecentAssessmentLogItem = AssessmentLogRecord;
...
export function useTeacherLibrary({
  ...
  const [recentAssessmentLogs, setRecentAssessmentLogs] = useState<RecentAssessmentLogItem[]>([]);
  const [recentAssessmentLogsStatus, setRecentAssessmentLogsStatus] = useState<"idle" | "loading" | "loaded" | "error">("idle");
  const [recentAssessmentLogsError, setRecentAssessmentLogsError] = useState("");
  ...
  useEffect(() => {
    let cancelled = false;
    if (mode !== "teacher" || !authUserId) {
      setRecentAssessmentLogs([]);
      setRecentAssessmentLogsStatus("idle");
      setRecentAssessmentLogsError("");
      return;
    }

    setRecentAssessmentLogsStatus("loading");
    supabase
      .from("assessment_logs")
      .select("id, owner_id, teacher_id, class_id, folder_id, student_id, assignment_id, exercise_id, exercise_title, seed, assessment_mode, weighted_score, total_possible, percent, correct_count, ambiguous_count, low_confidence_count, incorrect_count, recovery_kind, tonal_state_kind, signal_quality_level, signal_quality_score, summary_text, note_details, summary_json, created_at")
      .order("created_at", { ascending: false })
      .limit(10)
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error) {
          const missingAssessmentLogsTable =
            error.code === "PGRST205" ||
            error.message.toLowerCase().includes("assessment_logs") ||
            error.message.toLowerCase().includes("could not find the table");
          if (missingAssessmentLogsTable) {
            setRecentAssessmentLogs([]);
            setRecentAssessmentLogsStatus("loaded");
            setRecentAssessmentLogsError("");
            return;
          }
          setRecentAssessmentLogsStatus("error");
          setRecentAssessmentLogsError(error.message);
          return;
        }
        setRecentAssessmentLogs((data ?? []) as RecentAssessmentLogItem[]);
        setRecentAssessmentLogsStatus("loaded");
        setRecentAssessmentLogsError("");
      });
  ...
  return {
    ...
    recentAssessmentLogs, recentAssessmentLogsStatus, recentAssessmentLogsError,
    // Batch
